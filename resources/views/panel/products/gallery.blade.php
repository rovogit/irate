...
@section('title')Галерея: {{ $product->title }}@endsection
@section('description')Галерея: {{ $product->title }}@endsection
    ...
    <nav>
        <ul class="breadcrumb breadcrumb-arrow">
            ...
            <li class="breadcrumb-item active">
                <a href="{{ route('panel.products.edit', $product) }}">{{ $product->title }}</a></li>
    ...
    <div class="nk-block-head nk-block-head-sm">
        <div class="nk-block-between">
            <div class="nk-block-head-content">
                <h3 class="nk-block-title page-title d-flex align-items-end">Галерея: {{ $product->title }}</h3>
            </div>
        </div>
    </div>
                ...
                <div id="s-gallery" class="c-drop" data-gallery="{{ $product->photos->pluck('path') }}">
                    @foreach($product->photos as $photo)
                        <div class="c-drop-item">
                            ...
                            <button class="btn btn-sm btn-outline-danger c-drop-btn" data-delete="{{ $photo->path }}">Удалить</button>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="card card-bordered">
            <div class="card-inner text-center">
                <div id="m-g" class="btn-group" aria-label="Manage gallery" style="display:none">
                    <button type="button" class="btn btn-primary" data-action="save">Сохранить изменения</button>
                    <button type="button" class="btn btn-dim btn-light" data-action="cancel">Отмена</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card card-bordered">
            <div class="card-inner">
                <div class="form-group">
                    <label class="form-label">Загрузить картинки в галерею</label>
                    <div class="form-control crutch-dropzone dz-clickable">
                        <div class="dz-message" data-dz-message>
                            <div>
                                <span class="dz-message-text">Перетащите картинки сюда</span>
  ...
  $(function () {
    ...
    const motor = {
      ...
      save: function (b) {
        ...
        $.post('{{ route('panel.products.gallery.update', $product) }}', { gallery: img }, null, 'json')
          .done(function (json) {
            if ('redirect' in json) {
